...
# Security v3 defaults
NEW_ACCOUNT_ALERT_HOURS = 72
RAID_AUTO_LOCKDOWN = True
SCAM_PHRASES = {
    "free robux", "free robux giveaway", "claim robux", "robux generator",
    "steam gift", "nitro gift", "free nitro", "discord nitro free",
    "verify your account here", "limited time giveaway",
}
SAFE_LINK_DOMAINS = {"roblox.com", "www.roblox.com", "youtube.com", "www.youtube.com", "youtu.be"}
...
data = {
    ...
    "exempt_channels": [],
    "lockdown_overwrites": {},
    "stats": {"deleted_messages": 0, "spam_blocks": 0, "invite_blocks": 0, "scam_blocks": 0, "warnings": 0, "timeouts": 0, "raid_alerts": 0, "lockdowns": 0, "new_account_alerts": 0},
}
...
def url_domain(url: str):
    cleaned = re.sub(r"^https?://", "", url, flags=re.I).split("/", 1)[0].lower().strip(".")
    if cleaned.startswith("www."):
        cleaned = cleaned[4:]
    return cleaned


def contains_scam_phrase(text: str):
    lowered = normalize(text)
    return next((phrase for phrase in SCAM_PHRASES if phrase in lowered), None)


async def set_real_lockdown(guild: discord.Guild, actor: str, automatic: bool = False):
    if data.get("lockdown_overwrites"):
        data["raid_mode"] = True
        save_data()
        return 0
    ...
    for channel in guild.text_channels:
        if channel.id in {SECURITY_LOG_CHANNEL_ID, SECURITY_COMMANDS_CHANNEL_ID} or is_exempt_channel(channel):
            continue
        ow = channel.overwrites_for(everyone)
        saved[str(channel.id)] = ow.send_messages
        try:
            await channel.set_permissions(everyone, send_messages=False, reason=f"Lumber Yard Security lockdown by {actor}")
            changed += 1
        except (discord.Forbidden, discord.HTTPException):
            pass
    data["lockdown_overwrites"] = saved
    data["raid_mode"] = True
    stat_inc("lockdowns")
    save_data()
    await security_log(log_embed("🚨 Server Lockdown", "Public text channels were locked." if changed else "Lockdown was triggered, but no channel permissions could be changed.", discord.Color.red(), Trigger=actor, Automatic="Yes" if automatic else "No", Channels=changed))
    return changed


async def clear_real_lockdown(guild: discord.Guild, actor: str):
    saved = data.get("lockdown_overwrites", {}) or {}
    restored = 0
    for channel_id, previous in list(saved.items()):
        ...
        try:
            await channel.set_permissions(guild.default_role, overwrite=ow, reason=f"Lumber Yard Security unlock by {actor}")
            restored += 1
        except (discord.Forbidden, discord.HTTPException):
            pass
    data["lockdown_overwrites"] = {}
    data["raid_mode"] = False
    save_data()
    await security_log(log_embed("🟢 Server Unlocked", "Saved channel permissions were restored.", discord.Color.green(), Actor=actor, Channels=restored))
    return restored
...
async def moderate_message(message: discord.Message):
    ...
    scam = contains_scam_phrase(content)
    if scam:
        reason = f"Possible scam phrase ({scam})"
    ...
    if reason:
        ...
        if deleted:
            stat_inc("deleted_messages")
            if reason == "Unauthorized Discord invite": stat_inc("invite_blocks")
            if reason in {"Message flooding", "Repeated message spam"}: stat_inc("spam_blocks")
            if reason.startswith("Possible scam phrase"): stat_inc("scam_blocks")
...
async def on_member_join(member: discord.Member):
    ...
    if age < timedelta(hours=NEW_ACCOUNT_ALERT_HOURS):
        stat_inc("new_account_alerts")
        await security_log(log_embed(
            "🆕 New Account Alert",
            f"{member.mention} joined with a recently created account.",
            discord.Color.orange(),
            User=f"{member} (`{member.id}`)",
            Account_Age=f"{max(0, int(age.total_seconds() // 3600))} hours",
            Created=discord.utils.format_dt(member.created_at, style="F"),
            Action="Alert only — no automatic punishment",
        ))

    if not data.get("raid_alerts", True):
        return
    now = time.monotonic()
    recent_joins.append((now, member.id))
    while recent_joins and now - recent_joins[0][0] > RAID_WINDOW_SECONDS:
        recent_joins.popleft()
    if len(recent_joins) >= RAID_JOIN_LIMIT and not data.get("raid_mode", False):
        stat_inc("raid_alerts")
        await security_log(log_embed(
            ...
            Action="Automatic lockdown" if RAID_AUTO_LOCKDOWN else "Staff review required",
        ))
        if RAID_AUTO_LOCKDOWN:
            await set_real_lockdown(member.guild, "automatic raid detection", automatic=True)
        else:
            data["raid_mode"] = True
            save_data()
...
async def security_setup(interaction: discord.Interaction):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`/security stats` — View protection statistics\n"
...
async def security_lockdown(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    changed = await set_real_lockdown(interaction.guild, str(interaction.user), automatic=False)
    await interaction.followup.send(f"🚨 **Lockdown enabled.** Locked **{changed}** public text channel(s).", ephemeral=True)
...
async def security_unlock(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    restored = await clear_real_lockdown(interaction.guild, str(interaction.user))
    recent_joins.clear()
    await interaction.followup.send(f"🟢 **Lockdown disabled.** Restored **{restored}** channel(s).", ephemeral=True)

@security_group.command(name="stats", description="View Security protection statistics.")
@app_commands.checks.has_permissions(manage_guild=True)
async def security_stats(interaction: discord.Interaction):
    st = data.get("stats", {})
    embed = discord.Embed(title="📊 Lumber Yard Security Stats", color=discord.Color.blue())
    labels = [("Deleted Messages","deleted_messages"),("Spam Blocks","spam_blocks"),("Invite Blocks","invite_blocks"),("Scam Blocks","scam_blocks"),("Warnings","warnings"),("Timeouts","timeouts"),("Raid Alerts","raid_alerts"),("Lockdowns","lockdowns"),("New Account Alerts","new_account_alerts")]
    for label, key in labels:
        embed.add_field(name=label, value=str(st.get(key, 0)))
    await interaction.response.send_message(embed=embed, ephemeral=True)
